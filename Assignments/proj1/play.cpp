...
bool Play::summary (string off, string dwn, string togo, string ydline) {
  ...
  if (offense == off && down == currDwn) {
    if ((ydsTogo + 1) == ytg || (ydsTogo - 1) == ytg) {
      if (ytg >= (((float)ydLn *.10) - (float)ydLn) || ytg <= (((float)ydLn * .10) + (float)ydLn)) {
        return true;
      }
    }
  }
...
void Play::setRelevance(string min, string off, string def, string dwn, string togo, string ydline) {
  ...
  relevance = -(abs((float)Min-(float)minsRemain)*5/3 + abs((float)ydsTogo-(float)ytg) + abs((float)ydLn-(float)onYdln));

  if (def == defence)
    relevance += 100; 
}

float Play::getRelevance() {
  return relevance;
}
